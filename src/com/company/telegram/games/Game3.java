...
public class Game3 extends Game {
    ...
    public String execute(String data, User user) {
        ...
        if (user.getSteps() == 0 && parseInt(data) != user.getTargetNumber()) {
            ...
            user.setTote(0);
            return super.sendResponse(user, Visualizer.LOSER + user.getTargetNumber() + Visualizer.GAME_EXIT, GameStates.GREETING);
        }
        if (parseInt(data) != user.getTargetNumber() & parseInt(data) > user.getTargetNumber()) {
            return super.sendResponse(user, Visualizer.MORE + user.getSteps() + Visualizer.GAME_EXIT, GameStates.PLAY);
        }
        if (parseInt(data) != user.getTargetNumber() & parseInt(data) < user.getTargetNumber()) {
            return super.sendResponse(user, Visualizer.NO_MORE + user.getSteps() + Visualizer.GAME_EXIT, GameStates.PLAY);
        }
        if (parseInt(data) == user.getTargetNumber()) {
            user.setBank(user.getBank() + user.getTote() * 2);
            user.setSteps(0);
            return super.sendResponse(user, Visualizer.WIN + user.getBank(), GameStates.GREETING);
        }
        return super.sendResponse(user, Visualizer.TRY_CREATE + Visualizer.GAME_EXIT, GameStates.GREETING);
    }
}
