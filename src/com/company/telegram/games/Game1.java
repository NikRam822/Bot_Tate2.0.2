...
public class Game1 extends Game {
    ...
    public String execute(String data, User user) {
        ...
        if (parseInt(data) > 10) {
            return super.execute(user, Visualizer.ERROR_STEPS_MORE + Visualizer.GAME_EXIT, GameStates.CREATE_STEPS);
        }
        if (parseInt(data) <= 0) {
            return super.execute(user, Visualizer.ERROR_STEPS_NO_MORE + Visualizer.GAME_EXIT, GameStates.CREATE_STEPS);
        }

        user.setSteps(Integer.parseInt(data));


        return super.execute(user, Visualizer.DO_TOTE + Visualizer.GAME_EXIT, GameStates.DO_TOTE);
    }
}